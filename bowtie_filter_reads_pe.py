...
for d in db_list:
	if db_complete:#this says that the string is not empty (aka not the first)
		...
		command = "bowtie2 --local -x %s%s -p 16 -t -1 %s -2 %s --un-conc %s_unmapped_paired.fastq --al-conc %s_mapped_paired.fastq -S %s.sam" % (path_to_dbs, d, updated_for, updated_rev, new_base_name, new_base_name, new_base_name)
...
#		print command
		...
		command = "bowtie2 --local -x %s%s -p 16 -t -1 %s -2 %s --un-conc %s_unmapped_paired.fastq --al-conc %s_mapped_paired.fastq -S %s.sam" % (path_to_dbs, d, left_paired, right_paired, new_base_name, new_base_name, new_base_name)
		print "executing: " + command
		os.system(command)
		db_complete += abbrev_db
		updated_for = new_base_name + '_unmapped_paired.1.fastq'
