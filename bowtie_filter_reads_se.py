...
for d in db_list:
	if db_complete:#this says that the string is not empty (aka not the first)
		...
		command = "bowtie2 --local -x %s%s -p 16 -t -U %s --un %s_unmapped.fastq --al %s_mapped.fastq -S %s.sam" % (path_to_dbs, d, updated_sing, new_base_name, new_base_name, new_base_name)
...
#		print command
		...
		command = "bowtie2 --local -x %s%s -p 16 -t -U %s --un %s_unmapped.fastq --al %s_mapped.fastq -S %s.sam" % (path_to_dbs, d, singletons, new_base_name, new_base_name, new_base_name)
		print "executing: " + command
		os.system(command)
		db_complete += abbrev_db
		updated_sing = new_base_name + '_unmapped.fastq'
